...
averBif <- function(wLi, wLr,
  ...
  integralrEf <- Vectorize(function(w){integrate(rEf, w, 1, rel.tol=.Machine$double.eps^0.35)$value})
...
#  res1 <- optimize(averBif1, c(0.135278640450004, 0.135279), tol=.Machine$double.eps, maximum=T)$maximum
#  return((res1-wLr)^2)
#}
#f2 <- Vectorize(f1)
#
#begin <- proc.time()
#optimize(f2, c(0.135278640450004, 0.135279), tol=.Machine$double.eps)
#end <- proc.time()
